...
## Todo
- Confirm to the LLM that this types of data are what the departments collect and submit for analyzing the increase percentage in access to clean water and sanitation facilities
- After implementing all the data model for the lead department of "PEO" check which facts table reference to the same dimension "facilities" and analyze if it can be stored in one table to remove redundancy but also promots easier read operation
